...
func _input(event):
	# Do nothing if camera is not used
	if not is_current():
		return
	...
	if event is InputEventMouseButton:
		match event.button_index:
			BUTTON_WHEEL_UP: # Zooms in camera
				if get_fov() < 50.0: continue
				set_fov(get_fov() - _fov_multiplier)
			BUTTON_WHEEL_DOWN: # Zooms out camera
...
func _update_movement(delta):
	...
	# Increase movement speed and FOV multiplier when holding the Shift key
	if Input.is_action_just_pressed("game_walk"): # game_walk is Shift key
		_fov_multiplier += 5.0
